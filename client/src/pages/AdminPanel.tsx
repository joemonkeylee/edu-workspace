...
import { ArrowLeft, Scan, StopCircle, FolderOpen } from 'lucide-react';
  ...
  return (
    <div className="h-full flex flex-col bg-surface">
      <header className="bg-sidebar text-white px-6 py-4 flex items-center gap-4 flex-shrink-0">
        ...
        <h1 className="text-lg font-bold">后台管理 · PDF 扫描导入</h1>
      </header>
      ...
      <main className="flex-1 overflow-auto p-6 max-w-4xl mx-auto w-full">
        ...
        <div className="bg-gray-900 rounded-lg shadow overflow-hidden">
          <div className="text-gray-400 text-xs px-4 py-2 border-b border-gray-700 font-mono">
            实时日志控制台
          </div>
          <div className="font-mono text-sm p-4 h-80 overflow-auto scrollbar-thin">
            {logs.length === 0 && !scanning && (
              <div className="text-gray-500">等待开始扫描...</div>
            )}
            {logs.map((log, i) => (
              <div
                key={i}
