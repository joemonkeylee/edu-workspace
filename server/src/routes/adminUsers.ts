...
// Apply admin guard to all routes when auth is enabled
...
router.get('/', async (req: Request, res: Response) => {
  ...
  const [users, total] = await Promise.all([
    prisma.user.findMany({
      where,
      select: {
        id: true, phone: true, email: true, isAdmin: true, nickName: true,
...
router.post('/', async (req: Request, res: Response) => {
  ...
  const user = await prisma.user.create({
    data: { phone, password: hashed, email: email || null, isAdmin, nickName, maxDevices },
    select: {
      id: true, phone: true, email: true, isAdmin: true, nickName: true,
...
router.delete('/:id/devices/:tokenId', async (req: Request, res: Response) => {
  const id = parseInt(req.params.id);
...
router.get('/settings/auth', async (_req: Request, res: Response) => {
