# admin 首页工作台：可统计项盘点 + 打卡进度墙规划

> 目标：给后台做一个个人工作台（`/admin` 概览页）。门户首页（/books）另有安排，这里不动。
> 已完成（P1）：上「学习概览」4 指标 + 下「我的提交」整宽列表（草稿 / 已打回 / 已提交 / 已批改 全在一处，点击直达作业）。
...
| `client/src/components/admin/MyWorkspace.tsx` | 工作台组件，纵向两块：上「学习概览」4 指标一行，下「我的提交」整宽列表 |
| `client/src/components/admin/AdminDashboard.tsx` | `/admin` 概览页外壳（标题 + 工作台） |
| `client/src/App.tsx` | `/admin` index 由「重定向到 /admin/books」改为渲染概览页 |
| `client/src/components/admin/AdminLayout.tsx` | 左侧菜单首位加「概览」；NavLink 加 `end`，否则该条在所有子页面都高亮 |

要点：
- **门户首页（/books）保持原样**，筛选条、排序、编辑、批量删除一概没动。
- **书本筛选下拉**：数据源是当前用户做过作业的书的 distinct（`books` 字段，带每本作业数，按最近动过排序）。筛选对行、状态计数、概览四项指标同时生效；下拉选项**始终取自未筛选的整个范围**，否则选了一本书后列表就只剩一项。
...
```
┌──────────────────┬──────────────────┬──────────────────────┐
│ 草稿待提交 3      │ 待批改 2          │ 已批改 8   │ 已打回 1 │
├──────────────────────────────────────────────────────────┤
│ 我的提交  [全部 草稿 已打回 已提交 已批改]        [刷新]     │
│ 缩略图 作业标题 状态 书名 页数 时间  操作                  │
│ ...（整宽卡片，列表自然高度，卡片内不滚动）                 │
└──────────────────────────────────────────────────────────┘
（english 打卡墙等后续模块插在中间或往下追加新行）
```

位置：`/admin`（概览页），跟其他 admin 页面一样外面套 `p-6`，页面自己滚动。
- 上：学习概览（4 个指标平铺一行）；下：我的提交（整宽）—— 只占页面一部分，往后还有别的模块。

- 热力图 53 列 × 7 行，hover tooltip 显示当天分项（笔迹 3 / 提交 1 / 标注 2…）。
- 顶部三个数字：连续天数、本周活跃天、本月提交数。
- 窄屏（`< lg`）改为上下堆叠，热力图可横向滚动或折叠成一行摘要。
- 现在右列下半就是预留位，`/stats/activity` 落地后直接替换。
